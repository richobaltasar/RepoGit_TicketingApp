CREATE PROCEDURE SP_Get_LogSumBulananPertanggalPEMDA
	@Month nvarchar(max)
AS
BEGIN
	SET NOCOUNT ON;
	...
	set @persenTiket = (select Persentase from DataSettingReportforExternal where NamaLaporan='Setting Laporan PEMDA' and Category='Ticket')
	set @persenParkir = (select Persentase from DataSettingReportforExternal where NamaLaporan='Setting Laporan PEMDA' and Category='Parkir')
	set @persenFNB = (select Persentase from DataSettingReportforExternal where NamaLaporan='Setting Laporan PEMDA' and Category='F&B')
