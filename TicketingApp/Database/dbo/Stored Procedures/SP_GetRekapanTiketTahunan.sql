CREATE PROCEDURE [dbo].[SP_GetRekapanTiketTahunan]
	@SetTahunan nvarchar(max)
AS
BEGIN
	SET NOCOUNT ON;	
	
	select q.Bulan,q.NamaTicket+' ('+JenisTransaksi+')' Uraian,q.Qty,q.TotalAfterDiskon Jumlah ,JenisTransaksi into #temp
	from
	(
		select Month(s.Tanggal) Bulan,NamaTicket,sum(Harga) Harga,sum(Qty) Qty,sum(TotalAfterDiskon) TotalAfterDiskon,JenisTransaksi from
		(
			select Tanggal,NamaTicket,Harga,Qty,NamaDiskon,
			((Harga*Qty)*(Diskon/100)) TotalDiskon,
			((Harga*Qty)-((Harga*Qty)*(Diskon/100))) TotalAfterDiskon,
			JenisTransaksi
			from
			(
				select replace(CONVERT(VARCHAR(10), CONVERT(date, replace(left(a.Datetime,10),'/','-'), 105), 23),'-','/') Tanggal,
				...
				group by a.NamaTicket,a.Harga,a.NamaDiskon,Diskon,
				b.JenisTransaksi,
				replace(CONVERT(VARCHAR(10), CONVERT(date, replace(left(a.Datetime,10),'/','-'), 105), 23),'-','/')
			) z
		) s
		group by Month(s.Tanggal),NamaTicket,JenisTransaksi 
	) q
