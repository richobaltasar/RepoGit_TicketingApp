...
CREATE PROCEDURE [dbo].[SP_GetDashboard_V1_0_0_9]
...
BEGIN
	...
	and ComputerName = @ComputerName and Status = 1 and CashierBy = @Username
	...
	where replace(CONVERT(VARCHAR(10), CONVERT(date, replace(left(Datetime,10),'/','-'), 105), 23),'-','') = FORMAT(GETDATE() , 'yyyyMMdd') 
	and isnull(PayEmoney,0) = 0 and isnull(TotalDebit,0) = 0 and ComputerName = @ComputerName and Status = 1 and CashierBy = @Username	

	select @TicketPaySaldo = sum(isnull(PayEmoney,0)) from LogRegistrasiDetail
	where replace(CONVERT(VARCHAR(10), CONVERT(date, replace(left(Datetime,10),'/','-'), 105), 23),'-','') = FORMAT(GETDATE() , 'yyyyMMdd') 
	and  isnull(PayCash,0)= 0 and isnull(TotalDebit,0) = 0 and  ComputerName = @ComputerName and Status = 1 and CashierBy = @Username
	...
	where replace(CONVERT(VARCHAR(10), CONVERT(date, replace(left(Datetime,10),'/','-'), 105), 23),'-','') = FORMAT(GETDATE() , 'yyyyMMdd') 
	and  isnull(PayCash,0) > 0 and isnull(PayEmoney,0) > 0 and isnull(TotalDebit,0) = 0 and ComputerName = @ComputerName and Status = 1 and CashierBy = @Username
	...
	where NamaUser = @Username and NamaComputer = @ComputerName and Status = 1
