CREATE PROCEDURE [dbo].[SP_GetRekapanAsuramsiTahunan]
	@SetTahunan nvarchar(max)
AS
BEGIN
	SET NOCOUNT ON;

	select Month(s.Tanggal) Bulan,Uraian,sum(HargaSatuan) HargaSatuan,sum(Qty) Qty,sum(Jumlah) Jumlah,JenisTransaksi into #temp from
	(
		select CONVERT(VARCHAR(10), CONVERT(date, replace(left(Datetime,10),'/','-'), 105), 23) Tanggal,
		'Biaya Asuransi ('+JenisTransaksi+')' Uraian,(Asuransi/QtyTotalTiket) HargaSatuan,
