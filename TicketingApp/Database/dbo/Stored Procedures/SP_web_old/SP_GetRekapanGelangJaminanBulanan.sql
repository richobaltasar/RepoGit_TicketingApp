CREATE PROCEDURE [dbo].[SP_GetRekapanGelangJaminanBulanan]
...
BEGIN
	...
	(
		...
		'Saldo Jaminan untuk Gelang' as Uraian,q.Qty,
		(q.Qty*q.SaldoJaminanAfter) Jumlah,JenisTransaksi from 
		(
			select
			...
			CONVERT(VARCHAR(10), CONVERT(date, replace(left(Datetime,10),'/','-'), 105), 23)
		) q
	) w
	group by w.WEEK_OF_MONTH,w.Uraian,w.JenisTransaksi	
