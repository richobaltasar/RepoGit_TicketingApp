CREATE PROCEDURE [dbo].[SP_GetRekapanFnBTahunanEmoney]
	@SetTahunan nvarchar(max)
AS
BEGIN
	SET NOCOUNT ON;
	select Month(s.Tanggal) Bulan,isnull(Uraian,'Tenant Tidak Dikenali (ID : '+(CAST(KodeBarang AS VARCHAR(15)))+')') as  Uraian,'CASH' JenisTransaksi,
	sum(Jumlah) Jumlah,sum(Qtx) Qty,KodeBarang into #temp
	from
	(
		select a.IdItemsKeranjang,
		replace(CONVERT(VARCHAR(10), CONVERT(date, replace(left(a.Datetime,10),'/','-'), 105), 23),'-','/') Tanggal,a.KodeBarang,
		...
		where AccountNumber != ''
		and right(left(a.Datetime,10),4) = @SetTahunan
	)s
	group by Month(s.Tanggal),Uraian,KodeBarang
	create table #Db
	(
		no bigint,
		Uraian nvarchar(max),
