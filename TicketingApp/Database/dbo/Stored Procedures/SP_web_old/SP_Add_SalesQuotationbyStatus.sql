CREATE PROCEDURE [dbo].[SP_Add_SalesQuotationbyStatus]
	@IdQuotation bigint,
	@IdRab bigint,
...
BEGIN
	...
	BEGIN TRY  
		...
		if not exists(select*from DataQuotationVendor where IdRab = @IdRab and CompanyName = @CompanyName and Sales = @Sales and NumberQuotationByVendor = @NumberQuotationByVendor)
		begin
			...
			Success:
			insert into DataQuotationVendor
			(
				IdRab,
			...
			begin 
				...
				begin
					...
					begin
						...
						if not exists(select*from DataQuotationVendor_Item where NamaItem = @NamaItem and Category = @Category)
						begin
			...
			select  @Omessage='Sales  '+Perihal+' - '+CompanyName+' No. Quot :'+NumberQuotationByVendor+' berhasil ditambah',@IdQuotation = IdQuotation from DataQuotationVendor where IdQuotation = SCOPE_IDENTITY()
			
			exec dbo.SP_Function_SetLogActivity @IdUser=@IdUserBy,@NamaTable='DataQuotationVendor',@IdRow = @IdQuotation,@message=@Omessage,@Action='ADD'
	...
	END TRY  
	BEGIN CATCH  
		select 'Sorry SP_Add_SalesQuotationbyStatus error' as title, 'Code:'+STR(ERROR_NUMBER())+' -' +ERROR_MESSAGE() as message,'error' as status
	END CATCH;  
END
