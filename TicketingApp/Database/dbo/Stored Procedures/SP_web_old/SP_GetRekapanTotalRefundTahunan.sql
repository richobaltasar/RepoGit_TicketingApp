CREATE PROCEDURE [dbo].[SP_GetRekapanTotalRefundTahunan]
	@SetTahunan nvarchar(max)
AS
BEGIN
	SET NOCOUNT ON;
	select Month(w.Bulan) Bulan,w.Uraian Uraian,sum(w.Qty) Qty,sum(w.Jumlah) Jumlah,'Cash' JenisTransaksi into #temp from
	(
		select Month(q.Tanggal) Bulan,
		q.Uraian,sum(Qtx) Qty,
		sum(Jumlah) Jumlah
		from 
		(
			select 
			replace(CONVERT(VARCHAR(10), CONVERT(date, replace(left(Datetime,10),'/','-'), 105), 23),'-','/') Tanggal,'Total Refund' Uraian,
			sum(TotalNominalRefund) Jumlah, count(AccountNumber) Qtx
			from LogRefundDetail
			where right(left(Datetime,10),4) = @SetTahunan
			group by replace(CONVERT(VARCHAR(10), CONVERT(date, replace(left(Datetime,10),'/','-'), 105), 23),'-','/')
		) q
		group by q.Uraian,Tanggal
	) w
	group by w.Bulan,Uraian
